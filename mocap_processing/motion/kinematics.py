...
class Skeleton(object):
    ...
    def add_joint(self, joint, parent_joint):
        ...
        self.index_dof_start_end[joint.name] = (
            self.num_dofs, self.num_dofs + joint.info['dof']
        )
...
class Motion(object):
    ...
    def get_pose_by_time(self, time):
        ...
        if time == t1:
            return self.postures[frame1]
        if time == t2:
            return self.postures[frame2]
        assert (t2-t1) > 1.0e-5
        alpha = basics.clamp((time-t1)/(t2-t1), 0.0, 1.0)
        return interpolate_posture(
            alpha, self.postures[frame1], self.postures[frame2]
        )
    ...
    def load_bvh(
        self, file, scale, load_skel, load_motion, v_up_skel, v_face_skel,
        v_up_env
    ):
        words = None
                        ...
                        for joint in self.skel.joints:
                            ...
                            for channel in joint.info['bvh_channels']:
                                ...
                                if channel == 'xposition':
                                    value = scale*value
                                    T = np.dot(
                                        T, mmMath.getSE3ByTransV([value, 0, 0])
                                    )
                                elif channel == 'yposition':
                                    value = scale*value
                                    T = np.dot(
                                        T, mmMath.getSE3ByTransV([0, value, 0])
                                    )
                                elif channel == 'zposition':
                                    value = scale*value
                                    T = np.dot(
                                        T, mmMath.getSE3ByTransV([0, 0, value])
                                    )
                                elif channel == 'xrotation':
                                    value = value*math.pi/180.0
                                    T = np.dot(
                                        T, mmMath.SO3ToSE3(mmMath.rotX(value))
                                    )
                                elif channel == 'yrotation':
                                    value = value*math.pi/180.0
                                    T = np.dot(
                                        T, mmMath.SO3ToSE3(mmMath.rotY(value))
                                    )
                                elif channel == 'zrotation':
                                    value = value*math.pi/180.0
                                    T = np.dot(
                                        T, mmMath.SO3ToSE3(mmMath.rotZ(value))
                                    )
                                else:
...
def _read_motions(
    job_idx, scale, v_up_skel, v_face_skel, v_up_env, resample, fps, verbose
):
    res = []
...
def read_motion_parallel(
    files, num_worker, scale, v_up_skel, v_face_skel, v_up_env, resample, fps,
    verbose
):
    '''
