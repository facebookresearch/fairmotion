...
class Velocity(object):
    ...
    def get_velocity(self, key, local, R_ref=None):
        return np.hstack(
            [
                self.get_angular_velocity(key, local, R_ref),
...
class Motion(object):
    ...
    def append(self, motion, blend_length=0.5):
        ...
        for i in range(len(poses_new)):
            self.add_one_frame(
                copy.deepcopy(times_new[i]), copy.deepcopy(poses_new[i].data)
            )
    ...
    def load_bvh(
        self, file, scale, load_skel, load_motion, v_up_skel, v_face_skel, v_up_env
    ):
        words = None
...
def read_motion_parallel(
    files, num_worker, scale, v_up_skel, v_face_skel, v_up_env, resample, fps, verbose
):
    """
